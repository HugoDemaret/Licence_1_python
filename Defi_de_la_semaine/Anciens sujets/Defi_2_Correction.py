#Voici la correction du programme python du défi 2! 
...
def convert(liste):
    ...
    for i in liste:             #parcourt la liste afin de traiter chaque string qu'elle contient
        if i == '':
            ...
            sum = ''
            for j in i:
                number = str(ord(j)) #décompose la string en char, prend la valeur ASCII du char, et fait une concaténation avec les char précédents
                sum += str(number)
...
def quicksort(liste):
    if len(liste) < 1:
        ...
        pivot = liste.pop() #selectionne le pivot
    ...
    return quicksort(lower_than) + [pivot] + quicksort(greater_than) #récursivité de traitement
